...
/**
 * Test Case
 * 1. Create a winner (John) & a loser (Bills)
...
contract('MultiNumberBettingV4', function(accounts) {
  ...
  it("should assert true", function() {
    ...
    return MultiNumberBettingV4.deployed().then(function(instance){
      ...
      console.log("Winner Info= " +result);
      assert.isTrue((result[1]) == 'John Miller');

      // Add another winner (Frank)
      multi_number_betting_v4.guess(1,"Franks Hiller",{from:franks_address});
      // Get the get Last Winner Info
      return multi_number_betting_v4.getLastWinnerInfo.call();
  }).then(function(result){
    console.log("Winner Info= " +result);
       assert.isTrue((result[1]) == 'Franks Hiller');
       // Check if Bill is winning - should return false
       return multi_number_betting_v4.checkWinning(bills_address).call();
  }).then(function(result){

    assert.isTrue((result[1]) == 'Bob Davis');
  });
});
